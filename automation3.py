...
class preprocess:
    ...
    def msg_generate(message_list, msg_dict):
        ...
        for i in range(message_list.shape[0]):
            ...
            msg_dict['rdate'] = str(message_list['일자'][i])[:10].replace("-",'')
            msg_dict['rtime'] = "1900"
...
if msg_list is not None:
    message_result = pd.read_excel(msg_list)
    # message_result = preprocess.msg_processing(message)
    message_result = preprocess.msg_generate(message_result,msg_dict_result)
    for i in range(len(message_result)):
        st.write("{}번째 예약 문자".format(i+1))
        st.dataframe(message_result[i])
